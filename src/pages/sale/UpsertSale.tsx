...
const UpsertSale = () => {
  ...
  const [saleDetailsUpdateIndex, setSaleDetailsUpdateIndex] = useState(-1);
  ...
  const [getLatestSaleForProduct, { data: productLatestSale }] =
    useLazyGetLatestSaleForProductQuery();
  ...
  useEffect(() => {
    if (productLatestSale && saleDetailsUpdateIndex > -1) {
      setFieldValue(
        `saleItems[${saleDetailsUpdateIndex}].price`,
        productLatestSale.saleItem.price,
      );

      setSaleDetailsUpdateIndex(-1);
    }
  }, [setFieldValue, saleDetailsUpdateIndex, productLatestSale]);
  if (isLoading) {
    return (
      <Spinner
        thickness="4px"
          ...
          <Flex>
            <Box width={"100%"} gap="20px">
                ...
                >
                  <FormControl isInvalid={errors.clientId && touched.clientId}>
                    ...
                    <Select
                      ...
                      onChange={(op) => setFieldValue("clientId", op.value)}
                    />
              ...
              <Flex gap={"20px"} marginTop="20px">
                {values.saleItems?.map((si, siIndex) => {
                  ...
                  return (
                    ...
                    >
                      <FormControl
                        isInvalid={stockProductError && stockProductTouched}
                      >
                        ...
                        <Select
                          ...
                          onChange={(op) => {
                            if (op.value !== si.stockProduct) {
                              getLatestSaleForProduct({
                                stockProductId: op.value,
                              });
                              setSaleDetailsUpdateIndex(siIndex);
                            }
                            setFieldValue(
                              `saleItems[${siIndex}].stockProduct`,
                              op.value,
                        ...
                        />
                        <FormErrorMessage>{stockProductError}</FormErrorMessage>
                      </FormControl>
                      <FormControl isInvalid={priceError && priceTouched}>
                        <FormLabel>Price</FormLabel>
                        <NumberInput
                          value={si.price}
                          onChange={(value) => {
                            setFieldValue(`saleItems[${siIndex}].price`, value);
                          }}
                        >
                      ...
                      >
                        ...
                        <Select
                          ...
                          isDisabled={true}
                        />
                        <FormErrorMessage>{priceUnitError}</FormErrorMessage>
                      </FormControl>
                      <FormControl isInvalid={amountError && amountTouched}>
                        <FormLabel>Amount</FormLabel>
                        <NumberInput value={values.saleItems[siIndex]?.amount}>
                          <NumberInputField
                      ...
                      <FormControl
                        isInvalid={amountUnitError && amountUnitTouched}
                      >
                      ...
                      >
                        <Button
                          background="red.500"
                          onClick={() => {
                            const arrCopy = [...values.saleItems];
                            arrCopy.slice(siIndex, 1);
                            setFieldValue("saleItems", arrCopy);
                          }}
                        >
